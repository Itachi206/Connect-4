...
int CheckRow(int col){
  int i;
  for(i = brow - 1; i >= 0; i--){
    if(board[i][col] == '0' && board[i][col] != 'R' && board[i][col] != 'B') {
        return i;
    }      
  }
 return i;
...
bool winCheck(char p) {
    for (int i = 0; i < brow; i++) {
        for (int j = 0; j < bcol; j++) {
            if (board[i][j] == p && board[i+1][j + 1] == p && board[i+2][j + 2] == p && board[i+3][j + 3] == p) {//Diagonal left down 
                return true;
            }
            if (board[i][j] == p && board[i][j + 1] == p && board[i][j + 2] == p && board[i][j + 3] == p) {//Horizontal Check
                return true;
            }
            if (board[i][j] == p && board[i-1][j + 1] == p && board[i-2][j + 2] == p && board[i-3][j + 3] == p) {//Diagonal Right Up 
                return true;
            }
            if (board[i][j] == p && board[i - 1][j] == p && board[i - 2][j] == p && board[i - 3][j] == p) {//Vertical Check
                return true;
            }
        }
    }
